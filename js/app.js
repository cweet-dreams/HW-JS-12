...
// Event Delete
const deleteBtns = document.querySelectorAll('tbody');

deleteBtns.forEach(deleteBtn => deleteBtn.addEventListener('click', function(e) {
    e.preventDefault();
    const book = new Book();
    const ui = new UI;
    const title = deleteBtn.querySelector('tr').querySelectorAll('td')[0].textContent;
    const author = deleteBtn.querySelector('tr').querySelectorAll('td')[1].textContent;
    const id = deleteBtn.querySelector('tr').querySelectorAll('td')[2].textContent;
    const store = new Store();

   if(e.target.nodeName === 'BUTTON'){
     deleteBtn.querySelector('tr').remove();
     ui.showAlert('Book deleted!', 'success');
     store.deleteBook(title, author, id);
   }
}));
